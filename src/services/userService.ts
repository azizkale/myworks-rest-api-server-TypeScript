...
import { addRole } from "../middlewares/role_add";
import { addGroupToUser } from "../middlewares/addGroupToUser";
import { getRoles } from "../middlewares/role_getAll";
...
export class UserService {
  ...
  addParticipantToGroup = async (groupId: any, email: any, role: string) => {
    ...
    const nodeRef = admin
      .database()
      .ref(`groups/${groupId}/users/${userRecords.uid}`);
    return nodeRef.once(
      "value",
      async (snapshot) => {
        ...
        async (snapshot) => {
          if (snapshot.exists()) {
            ...
            from(Object.values(data))
              .pipe(
                concatMap((data: any) =>
                  this.retrieveUserByEmail(data.email).then(
                    (userRecord: any) => {
                      return {
