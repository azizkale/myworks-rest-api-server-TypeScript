...
export class GroupService {
      ...
      nodeRef.once(
        ...
        async (snapshot) => {
          ...
          if (snapshot.exists()) {
                ...
                concatMap(async (pirId: any) => {
                  ...
                  ref
                    ...
                    .then(async () => {
                      ...
                      await deleteGroupFromUsers(groupId).then(
                        async (data: any) => {
            ...
            //removes the group from users at first
            await deleteGroupFromUsers(groupId).then(async (data: any) => {
              //then deletes group from the node 'groups'
              const ref = await admin.database().ref("groups/");
              return await ref.child(groupId).remove();
            });
  ...
  async retrieveAllGroupsOfTheUserByuserId(userId: any): Promise<any[]> {
    return new Promise<any[]>((resolve, reject) => {
      this.getUsersAllGroupsAndRoles(userId)
        .then((info) => {
          const listGroups = info.val();
          from(Object.values(listGroups))
            .pipe(
              concatMap((data: any) =>
                this.retrieveSingleGroupByGroupId(data.groupId)
              ),
              map((group: Group | any) => ({
    ...
    admin
      ...
      .then((userRecords) => {
        ...
        return nodeRef.once("value", async (snapshot) => {
          ...
          } else return await { result: "user cannot be deleted" };
        });
      });
  }
}
function addGroupToUser(mentorId: any, groupId: any, arg2: string) {
