...
export class BookController {
  createBook = async (req: Request | any, res: Response | any) => {
  ...
  retrieveAllBooks = async (req: Request, res: Response) => {
    try {
      const token = req.headers["authorization"]?.split(" ")[1];
      ...
      const nodeRef = admin
        .database()
        .ref("users/" + response.uid + "/works/books/");
  ...
  updateBook = async (req: Request, res: Response) => {
    try {
      ...
      const ref = db.ref(
        "users/" + response.uid + "/works/books/" + book.bookId
      );
